...
**Важно: два разных HTTP-метода, не путать.**
...
| Ситуация | HTTP-статус от 1С | Результат в UI |
|----------|-------------------|----------------|
| Регистр существует, даты отсутствуют (ping) | `200` с `{"data": []}` | ✅ `ok` |
...
> Наш сервис считает соединение **успешным** при любом статусе кроме 401, 404 и 5xx.

**Полный обработчик в 1С** с обработкой ping и реального запроса в одной функции:
...
Функция ПолучитьДанные(Запрос) Экспорт
    ...
    // ── Ping-запрос: даты не переданы — это test connection, не запрос данных
    Если НЕ ЗначениеЗаполнено(СтрДатаНачала) ИЛИ НЕ ЗначениеЗаполнено(СтрДатаОкончания) Тогда
        Если Метаданные.РегистрыНакопления.Найти(ИмяРегистра) = Неопределено Тогда
            Возврат ОтветОшибка(404, "Регистр накопления не найден: " + ИмяРегистра);
        КонецЕсли;
        // Сервис жив, регистр существует — возвращаем пустой ответ
        Ответ = Новый HTTPСервисОтвет(200);
    ...
    // ── Валидация: все три параметра обязательны для реального запроса
    Если НЕ ЗначениеЗаполнено(ИмяРегистра) Тогда
        Возврат ОтветОшибка(400, "Обязательный параметр: register_name");
    КонецЕсли;

    // ── Проверяем существование регистра в метаданных
    МетаРегистр = Метаданные.РегистрыНакопления.Найти(ИмяРегистра);
    Если МетаРегистр = Неопределено Тогда
        Возврат ОтветОшибка(404, "Регистр накопления не найден: " + ИмяРегистра);
    КонецЕсли;

    // ... далее основная логика запроса и сериализации (см. раздел выше)
