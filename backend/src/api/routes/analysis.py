...
async def run_analysis(request: dict) -> dict:
    """
    Trigger an anomaly analysis for a specified data source and persist the run and detected anomalies.
    
    Parameters:
        request (dict): Request body containing `source_id`, `date_from`, and `date_to`.
    
    Returns:
        dict: Summary of the persisted analysis run with keys `analysis_id`, `status`, `anomaly_count`, `duration_ms`, and `source_id`.
    
    Raises:
        HTTPException: 422 if any of `source_id`, `date_from`, or `date_to` are missing;
                       400 if the specified source cannot be found.
    """
...
async def get_analysis_run(run_id: str) -> dict:
    """
    Retrieve a stored analysis run by its identifier.
    
    Returns the run serialized as a dictionary and formats date and timestamp fields as ISO strings when possible.
    ...
    Returns:
        dict: {
            "id": str,
            "source_id": str,
            "date_from": str or original value,
            "date_to": str or original value,
            "status": str,
            "anomaly_count": int,
            "started_at": str or None,
            "completed_at": str or None
        } — date and timestamp fields are ISO-formatted strings when applicable.
    
    Raises:
        HTTPException: 404 if the analysis run is not found; 500 for other unexpected errors with the error message.
    """
    try:
