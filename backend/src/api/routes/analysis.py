...
async def run_analysis(request: dict) -> dict:
    ...
    if not source_id or not date_from or not date_to:
        raise HTTPException(
            status_code=422,
            detail={"error": "missing_fields", "message": "source_id, date_from, and date_to are required"}
        )
    ...
    threshold_rules = ThresholdRules(
        ...
        spike_zscore=source.threshold_rules.get("spike_zscore", 3.0) if source.threshold_rules else 3.0,
        spike_logic=source.threshold_rules.get("spike_logic", "OR") if source.threshold_rules else "OR",
        moving_avg_window=source.threshold_rules.get("moving_avg_window", 6) if source.threshold_rules else 6,
        trend_window=source.threshold_rules.get("trend_window", 3) if source.threshold_rules else 3,
        zero_neg_enabled=source.threshold_rules.get("zero_neg_enabled", True) if source.threshold_rules else True,
        missing_enabled=source.threshold_rules.get("missing_enabled", True) if source.threshold_rules else True,
        ratio_enabled=source.threshold_rules.get("ratio_enabled", False) if source.threshold_rules else False,
        ratio_min=None,
        ratio_max=None,
    )
...
async def get_analysis_run(run_id: str) -> dict:
    ...
    try:
        ...
        return {
            ...
            "date_from": run.date_from.isoformat() if isinstance(run.date_from, (date, datetime)) else run.date_from,
            "date_to": run.date_to.isoformat() if isinstance(run.date_to, (date, datetime)) else run.date_to,
            "status": run.status,
